	    ...
	    //GImage img2 = new GImage(256,256);
	     ...
	     for(int i=0;i<256;i++) {
	    	 ...
	    	 for(int j=0;j<256;j++) {
	    		 ...
	    		 //img1.pixel[i][j]=img.pixel[i][j]-40;

	    		 if (img.pixel[i][j] > GImage.MIN_GRAY + 40 && img.pixel[i][j] < GImage.MAX_GRAY) {

	    			  img1.pixel[i][j]=img.pixel[i][j]-40;

	    			}
	    		 else if (img.pixel[i][j] >= GImage.MAX_GRAY) {
	    			 img1.pixel[i][j]=255;
	    		 }
	    		 else {
	    			 img1.pixel[i][j]=0;
